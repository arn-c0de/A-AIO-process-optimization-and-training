...
Last update: 2026-02-21 13:32:40 CET (+0100)

This file is auto-generated by `tools/update_architektur_md.sh`.

## How To Read

- Purpose: complete architecture and project structure snapshot.
- Focus: source/config/scripts/docs/GUI + concise runtime folder layout.
- Excluded noise: `__pycache__/`, `.venv/`, `.venv-wheelhouse/`, `.pytest_cache/`.
- Update command: `./tools/update_architektur_md.sh` (from `Simple-Sim/`).

## Architecture Conventions

- New CLI workflows: add entrypoints in `scripts/`; keep reusable logic in `simple_sim/`.
- New validation/migration helpers: place in `tools/`.
- New GUI features: use `gui/tabs/<feature>/` with `tab.py`, `logic.py`, and `ui.py` when applicable.
- Shared GUI primitives: place reusable widgets in `gui/components/` and cross-tab helpers in `gui/utils/`.
- New docs: add user-facing guides to `docs/guides/`, engineering notes to `docs/technical/`, and experiment logs to `docs/training_logs/`.
- Runtime outputs: write generated artifacts under `outputs/`; keep source folders free of run artifacts.
